...
if (path === '/api/health') {
  ...
  if (path.match(/^\/api\/deposits\/[^/]+\/upload-receipt$/) && method === 'POST') {
    ...
    if (!receipt) return res.status(400).json({ success: false, message: 'Receipt is required' })
    ...
    const { data: updatedDeposit } = await supabase
      .from('deposits')
      .update({ receipt_screenshot: receiptUrl, status: 'PAYMENT_SUBMITTED' })
      .eq('id', depositId)
      .select()
      .single()
    ...
    if (BOT_TOKEN && ADMIN_CHAT_ID && receiptUrl) {
      try {
        const bot = await initTelegramBot()
        const buttons = [
          { text: '✅ Confirm', callback_data: `confirm_payment_${depositId}` },
          { text: '❌ Reject', callback_data: `reject_payment_${depositId}` }
        ]
        const markup = { inline_keyboard: buttons.map(b => [{ text: b.text, callback_data: b.callback_data }]) }
        await sendTelegramPhoto(bot, receiptUrl, `💰 Payment Proof\nDeposit ID: ${depositId}`, markup)
      } catch (e) {
        console.error('Telegram error:', e)
      }
    }
  ...
  try {
    ...
    if (path === '/api/investments' && method === 'POST') {
      ...
      const { data: deposit, error: depositErr } = await supabase
        .from('deposits')
        .insert({ user_id: decoded.id, amount, payment_method: paymentMethod || 'ECOCASH', status: 'WAITING_FOR_PAYMENT_DETAILS' })
        .select()
        .single()
      ...
      const { data: investment, error: invErr } = await supabase
        .from('investments')
        .insert({
          user_id: decoded.id, plan_id: planId, deposit_amount: amount, current_balance: amount,
          status: 'PENDING', investment_id: `INV-${Date.now()}-${decoded.id.substring(0, 8)}`
        })
        .select()
        .single()

      if (invErr) throw invErr
      await supabase.from('deposits').update({ investment_id: investment.id }).eq('id', deposit.id)
    ...
    if (path === '/api/investments' && method === 'GET') {
      const decoded = getUserId(req)
      if (!decoded) return res.status(401).json({ success: false, message: 'Authorization required' })
      const { data, error } = await supabase.from('investments').select('*, plan:investment_plans(*), deposits(*)').eq('user_id', decoded.id).order('created_at', { ascending: false })
      if (error) throw error;
      return res.json({ success: true, data });
    }
    ...
    if (path.match(/^\/api\/investments\/[^/]+\/start-trade$/) && method === 'PUT') {
      ...
      if (error) throw error
      if (BOT_TOKEN && investment.user_id) {
        try {
          const { data: user } = await supabase.from('users').select('telegram_chat_id').eq('id', investment.user_id).single()
          const bot = await initTelegramBot()
          if (user?.telegram_chat_id) {
            await sendTelegramMessage(bot, `🚀 Trade Started! #${investment.investment_id}\nAmount: $${investment.deposit_amount}`, { chat_id: Number(user.telegram_chat_id) })
          }
        } catch (e) { console.error('Telegram error:', e) }
      }
    ...
    if (path.match(/^\/api\/deposits\/[^/]+\/send-details$/) && method === 'PUT') {
      ...
      const { data: deposit, error } = await supabase
        .from('deposits')
        .update({ ecocash_number: ecocashNumber, ecocash_account_name: ecocashAccountName, ecocash_reference: ecocashReference, status: 'PAYMENT_DETAILS_SENT' })
        .eq('id', depositId)
        .select('*, user:users(*)')
        .single()
      if (error) throw error
